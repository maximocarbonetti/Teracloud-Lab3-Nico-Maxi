...
resource "aws_ecr_lifecycle_policy" "this" {
  ...
  policy = jsonencode({
    rules = [
      {
        rulePriority = 1
        description  = "Borrar imagenes sin tag despues de ${var.untagged_expire_days} dias"
        selection = {
          tagStatus   = "untagged"
      ...
      {
        rulePriority = 2
        description  = "Retener solo las ultimas ${var.max_image_count} imagenes tageadas"
        selection = {
          tagStatus     = "tagged"
          tagPatternList = ["*"]
